...
static void internal_ArcDrawCharacterTaskbar(void)
{
    ...
    GuiSetTooltip("Go back to the main menu");
    ...
    GuiSetTooltip("Add a new character");
    ...
    GuiSetTooltip("Open a character file");
    ...
    GuiSetTooltip("Save the current character file");
    ...
    GuiSetTooltip("Load a spritesheet for the character");
...
static void internal_DrawCharacterProperties(void)
{
    ...
    const Rectangle windowRect = {
        (float)(GetScreenWidth() - windowWidth), 60, (float)windowWidth, (float)windowHeight - 60
    };
    ...
    GuiTextBox((Rectangle){(float)(GetScreenWidth() - windowWidth) + 80, 100, windowWidth - 100, 30}, columnText,
               sizeof(columnText), false);

    static char rowText[128] = "4";
    GuiLabel((Rectangle){(float)(GetScreenWidth() - windowWidth) + 10, 140, windowWidth - 20, 20}, "Rows:");
    GuiTextBox((Rectangle){(float)(GetScreenWidth() - windowWidth) + 80, 140, windowWidth - 100, 30}, rowText,
               sizeof(rowText), false);
    ...
    GuiTextBox((Rectangle){(float)(GetScreenWidth() - windowWidth) + 80, 180, windowWidth - 100, 30}, spriteWidthText,
               sizeof(spriteWidthText), false);
    ...
    GuiTextBox((Rectangle){(float)(GetScreenWidth() - windowWidth) + 80, 220, windowWidth - 100, 30}, spriteHeightText,
               sizeof(spriteHeightText), false);

    GuiWindowBox((Rectangle){(float)(GetScreenWidth() - windowWidth), 320, (float)windowWidth, 220},
                 "Character Values");

    // HP
    static char hpText[128] = "100";
    GuiLabel((Rectangle){(float)(GetScreenWidth() - windowWidth) + 10, 380, windowWidth - 20, 20}, "HP:");
    GuiTextBox((Rectangle){(float)(GetScreenWidth() - windowWidth) + 80, 380, windowWidth - 100, 30}, hpText,
               sizeof(hpText), false);

    // ATK
    static char attackText[128] = "50";
    GuiLabel((Rectangle){(float)(GetScreenWidth() - windowWidth) + 10, 420, windowWidth - 20, 20}, "Attack:");
    GuiTextBox((Rectangle){(float)(GetScreenWidth() - windowWidth) + 80, 420, windowWidth - 100, 30}, attackText,
               sizeof(attackText), false);

    // DEF
    static char defenseText[128] = "30";
    GuiLabel((Rectangle){(float)(GetScreenWidth() - windowWidth) + 10, 460, windowWidth - 20, 20}, "Defense:");
    GuiTextBox((Rectangle){(float)(GetScreenWidth() - windowWidth) + 80, 460, windowWidth - 100, 30}, defenseText,
               sizeof(defenseText), false);

    // SPD
    static char speedText[128] = "10";
    GuiLabel((Rectangle){(float)(GetScreenWidth() - windowWidth) + 10, 500, windowWidth - 20, 20}, "Speed:");
    GuiTextBox((Rectangle){(float)(GetScreenWidth() - windowWidth) + 80, 500, windowWidth - 100, 30}, speedText,
               sizeof(speedText), false);
}
...
static void internal_DrawFooterBar(void)
{
    ...
    const Rectangle footerRect = {
        0.0f, (float)(GetScreenHeight() - footerHeight), (float)GetScreenWidth(), (float)footerHeight
    };

    GuiPanel(footerRect, NULL);

    GuiLabel((Rectangle){10, GetScreenHeight() - footerHeight + 2, 80, 20}, "Zoom:");
    GuiTextBox((Rectangle){40, GetScreenHeight() - footerHeight + 2, 80, 20}, zoom, 32, false);

    GuiLabel((Rectangle){140, GetScreenHeight() - footerHeight + 2, 120, 20}, "Selected Cell:");
    GuiTextBox((Rectangle){220, GetScreenHeight() - footerHeight + 2, 60, 20}, TextFormat("%s", selectedCell), 32,
               false);
}

void ArcUICharacterSetZoomText(float* scale)
{
    snprintf(zoom, sizeof(zoom), "%.2f", *scale);
}

void ArcUICharacterSetSelectedCell(Vector2* pos)
{
    snprintf(selectedCell, sizeof(selectedCell), "%.2f %.2f", pos->x, pos->y);
}
